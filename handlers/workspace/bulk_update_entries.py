...
class WorkspaceBulkUpdateEntriesHandler(BaseHandler):
    async def post(self):
        try:
            ...
            update_payload = [
                (entry.id, msgspec.json.encode(entry.data).decode())
                for entry in updates
            ]

            updated_ids, job_name_map = await self.workspace_db.bulk_update_entries(
                update_payload
            )
            ...
            await self.workspace_db.invalidate_entries_cache_bulk(
                [e.id for e in updates if e.id in updated_ids]
            )
            ...
            request_urls = [
                f"/workspace/get_entries_by_name/{job_id}/{name}"
                for job_id, name_set in job_name_map.items()
                for name in name_set
            ]

            self.signal_clients_for_changes(
                self.get_client_name_from_header(), request_urls
            )
