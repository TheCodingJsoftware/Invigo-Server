            ...
            if "by_job" in view:
                if job_id:
                    data = await self.view_db.find_by_job(
                        job_id=int(job_id), group_id=int(group_id), name=name, flowtag=flowtag, flowtag_index=int(flowtag_index), data_type=data_type
                    )
                else:
                    self.send_error(400, reason="job_id is required for by_job view")
                    return
            else:  # global view
                data = await self.view_db.find_global(name=name, group_id=int(group_id), flowtag=flowtag, flowtag_index=int(flowtag_index), data_type=data_type)
