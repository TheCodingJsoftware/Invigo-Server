...
class Environment:
    ...
    POSTGRES_MAX_INACTIVE_CONNECTION_LIFETIME = int(
        os.getenv("POSTGRES_MAX_INACTIVE_CONNECTION_LIFETIME", 60)
    )
    PORT = int(os.getenv("PORT", 5057))
    DATA_PATH = os.getenv("DATA_PATH", "")
    WORKSPACE_BACKGROUND_CACHE_WARM_UP_INTERVAL = int(
        os.getenv("WORKSPACE_BACKGROUND_CACHE_WARM_UP_INTERVAL", 60)
    )
